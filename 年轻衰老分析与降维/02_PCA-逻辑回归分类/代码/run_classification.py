"""Subject-level binary classification with fixed parameters and ordinary five-fold CV."""
...
CV_SPLITS = 5
PARAMETER_SOURCE = "固定参数来自此前参数比较结果；本轮不搜索参数，仍有历史选择影响"


def build_model(model_name: str, pca_variance: float, seed: int):
    if model_name == "svm":
        return Pipeline(
            [
                ...
                (
                    "model",
                    SVC(kernel="linear", C=0.01, class_weight="balanced"),
                ),
            ]
        )
...
def fixed_parameters(model_name: str, pca_variance: float, seed: int) -> dict:
    if model_name == "svm":
        return {
...
def save_figures(
    ...
    ax.set_title(f"{model_title}: five-fold out-of-fold ROC")
    ax.grid(alpha=0.2)
    ...
    ax.set_title(f"{model_title}: five-fold out-of-fold confusion matrix")
    for row in range(2):
...
def five_fold_cross_validation(
    ...
    if np.bincount(labels, minlength=2).min() < CV_SPLITS:
        raise ValueError("Ordinary five-fold validation requires at least five subjects per class")
    ...
    for fold, (train_index, test_index) in enumerate(cv.split(matrix, labels), start=1):
        estimator = build_model(model_name, pca_variance, seed)
        estimator.fit(matrix[train_index], labels[train_index])
        fold_predictions = estimator.predict(matrix[test_index])
        fold_scores = score_estimator(estimator, matrix[test_index], model_name)
        ...
        fold_results.append(
            {
                ...
                "fixed_parameters": fixed_parameters(model_name, pca_variance, seed),
                **fold_metrics,
            }
        )

    if (predictions < 0).any() or np.isnan(scores).any():
        raise RuntimeError("Five-fold predictions are incomplete")
    ...
    # This sixth fit is the deployment model; it is not used for reported CV scores.
    final_model = build_model(model_name, pca_variance, seed)
    final_model.fit(matrix, labels)
    joblib.dump(final_model, model_output / "model.joblib")
    ...
    result = {
        ...
        "validation_method": "StratifiedKFold_fixed_parameters",
        "cv_splits": CV_SPLITS,
        "random_seed": seed,
        "parameter_search_performed": False,
        "fixed_parameters": fixed_parameters(model_name, pca_variance, seed),
        "parameter_source": PARAMETER_SOURCE,
        "final_model_note": "全数据最终模型与折内评价模型分开；报告指标全部来自折外预测",
        "metrics": metrics,
    }
