...
def classical_mds(distance: np.ndarray):
    ...
    component_count = min(2, len(values))
    if component_count:
        coordinates[:, :component_count] = (
            vectors[:, :component_count] * np.sqrt(values[:component_count])
        )
        shares[:component_count] = values[:component_count] / values.sum()
...
def main() -> None:
    ...
    if not 0 < args.perplexity < len(matrix):
        raise ValueError(
            f"t-SNE perplexity must be greater than 0 and smaller than the "
            f"number of subjects ({len(matrix)})"
        )
